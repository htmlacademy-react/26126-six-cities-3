    ...
    <ul className="locations__list tabs__list" data-testid="cities-container">
      {
        CITY_LOCATIONS.map((item)=>(
          ...
          >
            <Link
              data-testid="city-link" className={ item.name === searchCityParams ? 'locations__item-link tabs__item tabs__item--active' : 'locations__item-link tabs__item'} to={`${AppRoute.Main}?city=${item.name}`}
            >
              <span>{item.name}</span>
            </Link>
          </li>
